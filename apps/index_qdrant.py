...
def ollama_embed(
    base_url: str, model: str, text: str, max_chars: int = 1200
) -> list[float]:
...
def main() -> None:
    ...
    ap.add_argument(
        "--collection", default="", help="Override QDRANT_COLLECTION from .env"
    )
    ap.add_argument("--recreate", action="store_true")
    ap.add_argument(
        "--limit", type=int, default=0, help="Index only first N chunks (0 = all)"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=1200,
        help="Max chars per chunk sent to embedding model",
    )
    ap.add_argument(
        "--out-manifest",
        dest="out_manifest",
        default="",
        help="Path for index manifest json",
    )
    ap.add_argument(
        "--out-errors",
        dest="out_errors",
        default="",
        help="Path for per-chunk indexing errors jsonl",
    )
    args = ap.parse_args()
    ...
    try:
        ...
        out_manifest = (args.out_manifest or "").strip() or str(
            default_dir / f"index_manifest_{stamp}.json"
        )
        out_errors = (args.out_errors or "").strip() or str(
            default_dir / f"index_errors_{stamp}.jsonl"
        )
        ...
        with tm.span("probe_embedding"):
            probe = ollama_embed(
                ollama_base, embed_model, "vector_size_probe", max_chars=64
            )
        with tm.span("ensure_collection", recreate=bool(args.recreate)):
            ensure_collection(
                client, collection, vector_size=len(probe), recreate=args.recreate
            )
                ...
                with chunks_path.open("r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if total % 200 == 0:
                            ...
                            if rate is not None:
                                print(
                                    f"progress: total={total} upserted={upserted} skipped={skipped} rate={rate:.2f} chunks/s"
                                )
                            else:
                                print(
                                    f"progress: total={total} upserted={upserted} skipped={skipped}"
                                )
                        ...
                        try:
                            vec = ollama_embed(
                                ollama_base, embed_model, text, max_chars=args.max_chars
                            )
