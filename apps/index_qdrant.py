            ...
            with out_errors_path.open("w", encoding="utf-8") as err_fp:
                with chunks_path.open("r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        # Track missing fields (pre-enrichment) for QC
                        if not obj.get("chunk_id"):
                            missing_chunk_id += 1
                        if not (obj.get("source_url") or obj.get("source_path") or obj.get("source")):
                            missing_source += 1

                        # Deterministic, human-readable chunk identifier
                        sha8 = (chunk_sha256 or "")[:8]
                        chunk_id = obj.get("chunk_id") or f"{doc_id}:p{page}:c{chunk_index}:{sha8}"

                        # Prefer URL (if available), fallback to local path or doc_id
                        source = obj.get("source_url") or obj.get("source_path") or obj.get("source") or doc_id

                        # Curated metadata (kept compact but useful for filtering/debug)
                        meta = {
                            "doc_id": doc_id,
                            "page": page,
                            ...
                            "version_kind": obj.get("version_kind"),
                            "license_class": obj.get("license_class"),
                            "redistribution_allowed": obj.get("redistribution_allowed"),
                        }

                        # Keep original fields for traceability, plus standardized keys.
                        payload = dict(obj)
                        payload["text"] = text
